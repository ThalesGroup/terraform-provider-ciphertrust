...
- **`name` is immutable.** Once set, it cannot be changed in place. To rename a policy, destroy and recreate the resource.
- **Drift detection.** `Read()` re-hydrates all fields from CipherTrust Manager on every refresh. Out-of-band changes to mutable attributes (e.g. `effect`, `actions`) will be surfaced as a non-empty plan on the next `terraform plan`.
- **Out-of-band deletion.** If a policy is deleted directly on CipherTrust Manager (outside Terraform), the next `terraform plan` will detect the 404, remove the resource from state, and propose recreation.
