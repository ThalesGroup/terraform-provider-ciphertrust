...
- **Immutable fields**: `name` and `interface_type` cannot be changed after the interface is created. Attempting to change either attribute in a subsequent `terraform plan` will produce an immediate plan-time error. To change an immutable field, destroy and recreate the resource.
- **Out-of-band deletion**: If an interface is deleted directly via the CipherTrust Manager UI or API (outside Terraform), the next `terraform plan -refresh` will call the CM API, receive a 404 response, remove the resource from Terraform state, and schedule recreation. Run `terraform apply` to restore the interface.
- **Out-of-band attribute changes**: If an interface attribute (such as `mode`) is changed directly via the CM API without going through Terraform, the next `terraform plan -refresh` will read the new value from CM and surface it as a non-empty plan diff showing the divergence.
