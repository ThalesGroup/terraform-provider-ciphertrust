...
- **`policy_name` is immutable.** Once set, it cannot be changed in place. To rename the policy, destroy and recreate the resource. If omitted, the resource targets the built-in `global` password policy.
- **Drift detection.** `Read()` re-hydrates all configured numeric fields (e.g. `inclusive_min_digits`, `password_lifetime`) from CipherTrust Manager on every refresh. Out-of-band changes to these fields will be surfaced as a non-empty plan on the next `terraform plan`.
- **Out-of-band deletion.** If a custom password policy is deleted directly on CipherTrust Manager (outside Terraform), the next `terraform plan` will detect the 404, remove the resource from state, and propose recreation.
- **The `global` policy cannot be destroyed.** `terraform destroy` is a no-op for resources targeting `policy_name = "global"` (or when `policy_name` is omitted). To manage the global policy, update its attributes in place.
