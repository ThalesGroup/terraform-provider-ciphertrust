...
The following attributes can only be set at creation time and cannot be changed afterward. Attempting to change them in a subsequent `terraform plan` will produce a clear error without modifying or recreating the resource:

- `algorithm` — cryptographic algorithm (e.g. `aes`, `rsa`, `ec`). Changing this value requires destroying and recreating the key outside of Terraform.
- `key_size` — bit length of the key (e.g. `256` for AES-256). Must match the algorithm requirements.
- `curveid` — elliptic curve identifier for `ec` keys (e.g. `prime256v1`).
- `name` — friendly key name. Once created, the name is used as a stable identifier and cannot be changed via this provider.
- `object_type` — key object type (e.g. `Symmetric Key`, `Certificate`).
- `xts` — XTS/CBC-CS1 mode flag. Immutable after creation.
- `empty_material` — if `true`, key material is not generated. Immutable after creation.

## Drift Detection

This provider detects out-of-band changes to mutable attributes (`description`, `labels`, `aliases`, `meta`, `undeletable`, `unexportable`, `rotation_frequency_days`, `usage_mask`) on every `terraform plan`. If a mutable attribute is changed directly in CipherTrust Manager (e.g. via the API or CM UI), the next `terraform plan` will show a diff and `terraform apply` will converge the resource back to the Terraform-managed configuration.

If the key is deleted out-of-band (e.g. via the CM API), Read() detects the 404 response and removes the resource from Terraform state. The next `terraform plan` will propose to recreate the key.

**Known limitation — `meta` convergence**: CipherTrust Manager's key PATCH endpoint uses merge semantics for the `meta` object. Removing `meta` from the Terraform configuration does NOT clear `meta.owner_id` or other `meta` sub-fields on the server. The server retains the previous value, and Read() will detect a perpetual diff between the empty config and the non-null server value. To fully clear `meta`, you must do so via the CM API directly.

## Write-Only Attributes

The following attributes are sent to CipherTrust Manager on creation but are **never returned by the CM GET endpoint** and therefore cannot be read back into Terraform state:

- `material` — key material (for key import). Marked `Sensitive: true`. After creation, this field is preserved from prior state rather than re-read from the API.
- `password` — PKCS#12 password (for import). Marked `Sensitive: true`. Preserved from prior state.
- `wrap_*` fields — wrapping parameters for key import/export. Preserved from prior state.
- `mac_sign_*` fields — MAC/signature verification fields for import. Preserved from prior state.
- `empty_material` — server-generation flag. Preserved from prior state (CM GET omits this field).

## Revocation

Setting `revocation_reason` marks the key as revoked in CipherTrust Manager. **Revocation is a one-way operation**: once a key is revoked, it cannot be un-revoked via Terraform (or via the CM API). The `revocation_reason` and `revocation_message` fields are preserved from prior state — CM does not return them in GET responses.
...
- `compromise_date` (String) (Immutable) Date/time the object entered into the compromised state.
...
- `format` (String) (Immutable) This parameter is used while importing keys ('material' is not empty), and also when returning the key material after the key is created ('includeMaterial' is true).
...
- `generate_key_id` (Boolean) (Immutable) If specified as true, the key's keyId identifier of type long is generated. Defaults to false.
- `hkdf_create_parameters` (Attributes) (Immutable) Information which is used to create a Key using HKDF. (see [below for nested schema](#nestedatt--hkdf_create_parameters))
...
- `padded` (Boolean) (Immutable) This parameter determines the padding for the wrap algorithm while unwrapping a symmetric key,
...
- `wrap_hkdf` (Attributes) (Immutable) Information which is used to wrap a Key using HKDF. (see [below for nested schema](#nestedatt--wrap_hkdf))
- `wrap_key_id_type` (String) (Immutable) IDType specifies how the wrapKeyName should be interpreted.
- `wrap_key_name` (String) (Immutable) While creating a new key, If 'includeMaterial' is true, then only the key material will be wrapped with material of the specified key name. The response material property will be the base64 encoded ciphertext. For more details, view wrapKeyName in export parameters.
...
- `wrap_rsaaes` (Attributes) (Immutable) Parameters for wrapping a key using RSA AES Key Wrap Padding (RSA/RSAAESKEYWRAPPADDING). (see [below for nested schema](#nestedatt--wrap_rsaaes))
- `wrapping_encryption_algo` (String) (Immutable) It indicates the Encryption Algorithm information for wrapping the key. Format is : Algorithm/Mode/Padding. For example : AES/AESKEYWRAP. Here AES is Algorithm, AESKEYWRAP is Mode & Padding is not specified. AES/AESKEYWRAP is RFC-3394 & AES/AESKEYWRAPPADDING is RFC-5649. For wrapping private key, only AES/AESKEYWRAPPADDING is allowed. RSA/RSAAESKEYWRAPPADDING is used to wrap/unwrap asymmetric keys using RSA AES KWP method. Refer WrapRSAAES to provide optional parameters.
- `wrapping_hash_algo` (String) (Immutable) This parameter specifies the hashing algorithm used if wrappingMethod corresponds to mac/sign. In case of MAC operation, the hashing algorithm used will be inferred from the type of HMAC key(macSignKeyIdentifier).
- `wrapping_method` (String) (Immutable) This parameter specifies the wrapping method used to wrap/mac/sign the key material
- `xts` (Boolean) (Immutable) If set to true, then key created will be XTS/CBC-CS1 Key. Defaults to false. Key algorithm must be 'AES'.
