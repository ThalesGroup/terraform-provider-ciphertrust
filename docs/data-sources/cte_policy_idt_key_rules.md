...
- `limit` (Number) Maximum number of IDT key rules to return. If unset, all rules are returned (a warning is emitted if the result set is large).
- `skip` (Number) Number of IDT key rules to skip before returning results, for pagination. Defaults to 0.
