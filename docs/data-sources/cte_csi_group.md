...
- `limit` (Number) Maximum number of CSI groups to return. If unset, all CSI groups are returned (a warning is emitted if the result set is large).
- `skip` (Number) Number of CSI groups to skip before returning results, for pagination. Defaults to 0.
