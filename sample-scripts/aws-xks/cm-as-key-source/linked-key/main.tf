...
# Create linked external custom keystore
...
data "ciphertrust_aws_custom_keystore" "data_linked_xks_custom_keystore_data" {
  depends_on = [
    ciphertrust_aws_custom_keystore.linked_xks_custom_keystore_for_cm_as_source,
  ]
  id = ciphertrust_aws_custom_keystore.linked_xks_custom_keystore_for_cm_as_source.id
}
output "data_linked_xks_custom_keystore_data" {
  value = data.ciphertrust_aws_custom_keystore.data_linked_xks_custom_keystore_data
}
...
resource "ciphertrust_aws_xks_key" "xks_linked_key_with_cm_as_source_2" {
  depends_on = [
    ciphertrust_aws_custom_keystore.linked_xks_custom_keystore_for_cm_as_source,
    ciphertrust_cm_key.cm_aes_key,
  ...
  description = "desc for xks_linked_key_with_cm_as_source_2"
  alias = ["a53_cm_linked_key_2"]
  tags = {
    Tagkey1 = "TagValue1"
    Tagkey2 = "TagValue2"
  }
...
output "xks_linked_key_with_cm_as_source_2" {
  value = ciphertrust_aws_xks_key.xks_linked_key_with_cm_as_source_2
}

data "ciphertrust_aws_xks_key" "aws_key_data_source" {
  depends_on = [
    ciphertrust_aws_xks_key.xks_linked_key_with_cm_as_source_2,
  ]
  arn = ciphertrust_aws_xks_key.xks_linked_key_with_cm_as_source_2.arn
...
# Set undeletable = false for ciphertrust_cm_key resource before deleting the CM key
