...
func TestCiphertrustCTEPolicyLDTKeyRulesDataSource(t *testing.T) {
	...
	// (even with undeletable=false). LDT policies additionally require cte_versioned=true
	// which triggers async cleanup on CDSPaaS; the key delete races with the policy
	// reference teardown and returns 403. Needs a CDSPaaS-aware retry/wait strategy
	// in the key Delete function before re-enabling here.
	RequireCM(t)
