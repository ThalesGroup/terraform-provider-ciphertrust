...
// gcpResponse returns a JSON response for connections.GetGcpParamsFromResponse with
// only the specified fields set to non-empty values. All other string fields default
// to empty to avoid gjson duplicate-key issues.
...
// Test_CM_GetGcpParamsFromResponse_PlainDrift verifies that
// connections.GetGcpParamsFromResponse correctly refreshes plain attributes from the
// CM API response so that attribute drift (e.g. description changed in CM UI) is
...
func Test_CM_GetGcpParamsFromResponse_PlainDrift(t *testing.T) {
	t.Run("all plain fields are populated from the response", func(t *testing.T) {
		...
		var data connections.GCPConnectionTFSDK
		var diags diag.Diagnostics
		connections.GetGcpParamsFromResponse(response, &diags, &data)
...
// sentinel checked in connections.ResourceGCPConnection.Read so that OOB deletes are
// caught.
...
// Test_CM_GetGcpKeyFile_ResolvesInlineOrFileContent verifies
// connections.GetGcpKeyFile's behavior across inline JSON, whitespace, and
// filesystem paths (existing, empty, and nonexistent), since that resolution feeds
// the empty-value check added to Create/Update to prevent a silent no-op on an
// empty key_file.
func Test_CM_GetGcpKeyFile_ResolvesInlineOrFileContent(t *testing.T) {
	...
	t.Run("empty string resolves to empty", func(t *testing.T) {
		if got := connections.GetGcpKeyFile(ctx, ""); got != "" {
			t.Errorf("got %q, want empty", got)
		}
	})

	t.Run("whitespace-only string resolves to empty", func(t *testing.T) {
		if got := connections.GetGcpKeyFile(ctx, "   \t\n  "); got != "" {
			t.Errorf("got %q, want empty", got)
		}
	})
	...
	t.Run("inline JSON that is not a filesystem path passes through verbatim", func(t *testing.T) {
		...
		if got := connections.GetGcpKeyFile(ctx, inline); got != inline {
	...
	t.Run("path to an existing non-empty file returns its content", func(t *testing.T) {
		...
		if got := connections.GetGcpKeyFile(ctx, path); got != content {
	...
	t.Run("path to an existing but empty file returns empty", func(t *testing.T) {
		...
		if got := connections.GetGcpKeyFile(ctx, path); got != "" {
			t.Errorf("got %q, want empty", got)
		}
	})
	...
	t.Run("path to a nonexistent file passes the raw path through verbatim", func(t *testing.T) {
		...
		if got := connections.GetGcpKeyFile(ctx, path); got != path {
...
// connections.ResolveGcpKeyFile returns an error instead of an empty resolved value
// for every input that GetGcpKeyFile collapses to "", closing the silent no-op gap
// left by validating only the raw key_file string at plan time.
func Test_CM_ResolveGcpKeyFile_RejectsEmptyResolvedValue(t *testing.T) {
	ctx := context.Background()

	t.Run("empty string is rejected", func(t *testing.T) {
		resolved, errMsg := connections.ResolveGcpKeyFile(ctx, "")
		if errMsg == "" {
	...
	t.Run("path to an existing but empty file is rejected", func(t *testing.T) {
		...
		resolved, errMsg := connections.ResolveGcpKeyFile(ctx, path)
		if errMsg == "" {
			t.Fatalf("expected an error message, got none (resolved=%q)", resolved)
		}
		if resolved != "" {
	...
	t.Run("non-empty inline JSON resolves without error", func(t *testing.T) {
		inline := `{"type":"service_account","client_email":"svc@project.iam.gserviceaccount.com"}`
		resolved, errMsg := connections.ResolveGcpKeyFile(ctx, inline)
		if errMsg != "" {
...
// gcpStringValidatorsAttribute is satisfied by schema.StringAttribute; used to pull
// the configured Validators back out of the built schema without depending on the
// concrete attribute struct.
type gcpStringValidatorsAttribute interface {
...
func Test_CM_GCPConnection_CloudNameEnumValidator(t *testing.T) {
	...
	(&connections.ResourceGCPConnection{}).Schema(ctx, fwresource.SchemaRequest{}, &schemaResp)
	...
	withValidators, ok := attr.(gcpStringValidatorsAttribute)
	if !ok {
