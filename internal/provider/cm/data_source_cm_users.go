...
func (d *dataSourceUsers) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, user := range users {
		userState := CMUserTFSDK{
			...
			LoginsCount:            types.Int64Value(user.LoginsCount),
			CertificateDN:          types.StringValue(user.CertificateDN),
			FailedLoginsCount:      types.Int64Value(user.FailedLoginsCount),
		}
