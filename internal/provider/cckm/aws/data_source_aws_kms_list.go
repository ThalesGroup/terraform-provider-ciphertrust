				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"connection_id": schema.StringAttribute{
							Computed:    true,
							Description: "CipherTrust Manager connection ID which manages this account.",
						},
...
func (d *dataSourceAWSKms) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for ndx, kms := range kmsList.Resources {
		kmsTFSDK := KMSModelTFSDK{
			...
			ConnectionID:         types.StringValue(kms.Connection),
			ConnectionName:       types.StringValue(kms.Connection),
