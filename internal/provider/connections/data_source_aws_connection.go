...
func (d *dataSourceAWSConnection) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, aws := range awsConnections {
		awsConn := AWSConnectionModelTFSDK{
			...
			Name:                    derefString(aws.Name),
			Description:             derefString(aws.Description),
			AccessKeyID:             derefString(aws.AccessKeyID),
			AssumeRoleARN:           derefString(aws.AssumeRoleARN),
			AssumeRoleExternalID:    derefString(aws.AssumeRoleExternalID),
			AWSRegion:               derefString(aws.AWSRegion),
			AWSSTSRegionalEndpoints: derefString(aws.AWSSTSRegionalEndpoints),
			CloudName:               derefString(aws.CloudName),
			IsRoleAnywhere:          derefBool(aws.IsRoleAnywhere),
			SecretAccessKey:         derefString(aws.SecretAccessKey),
